...
RSpec.describe HappiMail::BodyParser, type: :model do
  ...
  it "works for plain text emails" do
    ...
    expect(HappiMail::BodyParser.new(mail.mail).content).to include("What's the story?")
  end
  ...
  it "parses out plain text replies" do
    ...
    expect(HappiMail::BodyParser.new(mail.mail).content).to eq("<p>What's the story?</p>")
  end
  ...
  it "adds line breaks to plain text" do
    ...
    expect(HappiMail::BodyParser.new(mail.mail).content).to eq("<p>Please join us for a party at</p>\n\n<p>Bag End</p>")
  end

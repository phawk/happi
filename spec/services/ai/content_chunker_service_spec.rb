...
RSpec.describe Ai::ContentChunkerService, type: :service do
  describe "#call" do
    ...
    let(:service) { described_class.new(content:, chunk_size: 100) }
    ...
    context "when the service is successful" do
      ...
      it "splits long paragraphs by punctuation" do
        result = service.call
        chunks = result.value!
        expect(chunks.any? { |chunk| chunk.start_with?("This is a test content.") }).to be true
        expect(chunks.any? { |chunk| chunk.start_with?("Here is another paragraph") }).to be true
        expect(chunks.any? { |chunk| chunk.start_with?("It contains multiple sentences") }).to be true
        expect(chunks.any? { |chunk| chunk.start_with?("This sentence is added") }).to be true
      end
    end
